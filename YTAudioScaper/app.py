...
def app(environ, start_response):
    ...
    if method == 'GET' and path == '/':
        ...
        # Decode URL from the form (we now properly decode it)
        size = int(environ.get('CONTENT_LENGTH', 0))
        body = environ['wsgi.input'].read(size).decode('utf-8')
        post_data = parse_qs(body)
        user_input = unquote(post_data.get('input_text', [''])[0])  # Decode the URL

        try:
            # Run the script with the decoded URL
            ...
            # Serve the file as an attachment (browser prompts 'Save As' dialog)
